...
public class ReceiveMain {
    public static List readMail(UserVo userVo){
        ...
        try{
            ...
            else parseMessage(userVo,list,messages); //解析邮件
            //释放资源
     ...
     */
    private static void parseMessage(UserVo user,List list, javax.mail.Message...messages) throws MessagingException, IOException {
        ...
        for (int i = 0, count = messages.length; i < count; i++) {
            ...
            if (isContainAttachment(msg)) {//保存附件
                String path = ReceiveMain.class.getResource("/").getPath().replaceAll("WEB-INF/classes","upload/recive");
     ...
     * @throws MessagingException
     */
    private static String getReceiveAddress(MimeMessage msg, javax.mail.Message.RecipientType type) throws MessagingException {
     ...
     * @throws IOException
     */
    private static void saveAttachment(Part part, String destDir,String email,String sendName,List<String> pathlist) throws UnsupportedEncodingException, MessagingException,
        ...
        if (part.isMimeType("multipart/*")) {
            ...
            for (int i = 0; i < partCount; i++) {
                ...
                if (disp != null && (disp.equalsIgnoreCase(Part.ATTACHMENT) || disp.equalsIgnoreCase(Part.INLINE))) {
                    ...
                    saveFile(is, destDir,filename ,email,sendName);
                } else if (bodyPart.isMimeType("multipart/*")) {
                    ...
                    if (contentType.indexOf("name") != -1 || contentType.indexOf("application") != -1) {
                        String filename=decodeText(bodyPart.getFileName());
                        pathlist.add(filename);
                        saveFile(bodyPart.getInputStream(), destDir, filename,email,sendName);
                    }
                }
            }
        } else if (part.isMimeType("message/rfc822")) {
