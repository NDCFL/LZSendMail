...
$('#mytab').bootstrapTable({
    ...
    url:"/mailModule/mailModuleList",//要请求数据的文件路径
    ...
    columns:[
        ...
        ,
        {
            ...
            formatter: function (value, row, index) {
                var e = '<a title="编辑" href="javascript:void(0);"  data-id="\'' + row.id + '\'"  onclick="return edit(\'' + row.id + '\')"><i class="glyphicon glyphicon-pencil" alt="修改" style="color:green"></i></a> ';
...
function del(mailModuleid,status){
    if(status==0){
    ...
    layer.confirm('确认要删除吗？',function(index){
        $.ajax({
            type: 'POST',
            url: '/mailModule/deleteMailModule/'+mailModuleid,
            dataType: 'json',
...
$('#search_btn').click(function(){
    $('#mytab').bootstrapTable('refresh', {url: '/mailModule/mailModuleList'});
})
function refush(){
    $('#mytab').bootstrapTable('refresh', {url: '/mailModule/mailModuleList'});
}
$("#update").click(function(){
    $.post(
        "/mailModule/mailModuleUpdateSave",
...
function deleteMany(){
    ...
    layer.confirm('确认要执行批量删除用户信息数据吗？',function(index){
        $.post(
            "/mailModule/deleteManyMailModule",
            {
                "manyId":$("#deleteId").val()
            },
            function(data){
