...
$('#mytab').bootstrapTable({
    ...
    url:"/houseType/houseTypeList",//要请求数据的文件路径
    ...
    columns:[
        ...
        ,
        {
            ...
            field:'',
            formatter: function (value, row, index) {
                var e = '<a title="编辑" href="javascript:void(0);" id="houseType"  data-toggle="modal" data-id="\'' + row.id + '\'" data-target="#myModal" onclick="return edit(\'' + row.id + '\')"><i class="glyphicon glyphicon-pencil" alt="修改" style="color:green"></i></a> ';
...
function del(houseTypeid,status){
    if(status==0){
    ...
    layer.confirm('确认要删除吗？',function(index){
        $.ajax({
            type: 'POST',
            url: '/houseType/deleteHouseType/'+houseTypeid,
            dataType: 'json',
...
function edit(name){
    $.post("/houseType/findHouseType/"+name,
        function(data){
...
function updatestatus(id,status){
    $.post("/houseType/updateStatus/"+id+"/"+status,
        function(data){
            if(status==0){
                if(data.message=="ok"){
