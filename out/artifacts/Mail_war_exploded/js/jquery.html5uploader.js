...
$.fn.html5uploader = function(opts){
	...
	this.each(function(){
		var _this = $(this);
		//先添加上file按钮和上传列表
            var inputstr = '<form action="/file/upload" method="post" enctype="multipart/form-data"><input class="uploadfile" style="visibility:hidden;" id="file" type="file" name="file"';
            if(option.multi){
			inputstr += 'multiple';
			}
		inputstr += '/><input type="submit" value="submit" id="sub"/></form>';
			  ...
			  funUploadFile: function(file) {
				  ...
				  (function(file) {
					  ...
					  if (xhr.upload) {
						  ...
						  // 开始上传
						  xhr.open("POST", self.url+"?file="+file, true);
