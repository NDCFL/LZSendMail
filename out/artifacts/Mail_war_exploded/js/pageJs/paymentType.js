...
$('#mytab').bootstrapTable({
    ...
    url:"/paymentType/paymentTypeList",//要请求数据的文件路径
    ...
    columns:[
        ...
        ,
        {
            ...
            field:'',
            formatter: function (value, row, index) {
                var e = '<a title="编辑" href="javascript:void(0);" id="paymentType"  data-toggle="modal" data-id="\'' + row.id + '\'" data-target="#myModal" onclick="return edit(\'' + row.id + '\')"><i class="glyphicon glyphicon-pencil" alt="修改" style="color:green"></i></a> ';
...
function del(paymentTypeid,status){
    if(status==0){
    ...
    layer.confirm('确认要删除吗？',function(index){
        $.ajax({
            type: 'POST',
            url: '/paymentType/deletePaymentType/'+paymentTypeid,
            dataType: 'json',
...
function edit(name){
    $.post("/paymentType/findpaymentType/"+name,
        function(data){
...
function updatestatus(id,status){
    $.post("/paymentType/updateStatus/"+id+"/"+status,
        function(data){
            if(status==0){
                if(data.message=="ok"){
...
$('#search_btn').click(function(){
    $('#mytab').bootstrapTable('refresh', {url: '/paymentType/paymentTypeList'});
})
function refush(){
    $('#mytab').bootstrapTable('refresh', {url: '/paymentType/paymentTypeList'});
}
$("#update").click(function(){
    $.post(
        "/paymentType/paymentTypeUpdateSave",
...
$("#add").click(function(){
    $.post(
        "/paymentType/paymentTypeAddSave",
        $("#formadd").serialize(),
        function(data){
