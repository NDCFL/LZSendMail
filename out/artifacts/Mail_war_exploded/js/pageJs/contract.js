...
$('#mytab').bootstrapTable({
    ...
    url:"/contract/contractList",//要请求数据的文件路径
...
function edit(name){
    $.post("/contract/findContract/"+name,
        function(data){
            //刷新列表
            $("#updateform").autofill(data);
            var colum = $("#masterById").select2();
            //选中某一行
            colum.val(data.contractMasterVo.id).trigger("change");
            colum.change();
            $("#contractDate").val(formatTime(data.contractDate));
            $("#payDate").val(formatTime(data.payDate));
        },
        "json"
    );
}
function updatestatus(id,status){
    $.post("/contract/updateStatus/"+id+"/"+status,
        function(data){
            if(status==0){
                if(data.message=="ok"){
...
$('#search_btn').click(function(){
    $('#mytab').bootstrapTable('refresh', {url: '/contract/contractList'});
})
function refush(){
    $('#mytab').bootstrapTable('refresh', {url: '/contract/contractList'});
}
