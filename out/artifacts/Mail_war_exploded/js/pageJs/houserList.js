...
$('#mytab').bootstrapTable({
    ...
    url: "/houser/houserList",//要请求数据的文件路径
...
function del(houser, status) {
    ...
    layer.confirm('确认要删除吗？', function (index) {
        $.ajax({
            type: 'POST',
            url: '/houser/deleteHouser/' + houser,
            dataType: 'json',
...
function updatestatus(id, status) {
    $.post("/houser/updateStatus/" + id + "/" + status,
        function (data) {
            if (status == 0) {
                if (data.message == "ok") {
...
$('#search_btn').click(function () {
    $('#mytab').bootstrapTable('refresh', {url: '/houser/houserList'});
})
function refush() {
    $('#mytab').bootstrapTable('refresh', {url: '/houser/houserList'});
}
