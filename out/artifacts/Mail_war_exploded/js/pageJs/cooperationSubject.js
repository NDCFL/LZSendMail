...
$('#mytab').bootstrapTable({
    ...
    url:"/cooperationSubject/cooperationSubjectList",//要请求数据的文件路径
    ...
    columns:[
        ...
        ,
        {
            ...
            field:'',
            formatter: function (value, row, index) {
                var e = '<a title="编辑" href="javascript:void(0);" id="cooperationSubject"  data-toggle="modal" data-id="\'' + row.id + '\'" data-target="#myModal" onclick="return edit(\'' + row.id + '\')"><i class="glyphicon glyphicon-pencil" alt="修改" style="color:green"></i></a> ';
...
function del(cooperationSubjectid,status){
    if(status==0){
    ...
    layer.confirm('确认要删除吗？',function(index){
        $.ajax({
            type: 'POST',
            url: '/cooperationSubject/deleteCooperationSubject/'+cooperationSubjectid,
            dataType: 'json',
...
function edit(name){
    $.post("/cooperationSubject/findCooperationSubject/"+name,
        function(data){
...
function updatestatus(id,status){
    $.post("/cooperationSubject/updateStatus/"+id+"/"+status,
        function(data){
            if(status==0){
                if(data.message=="ok"){
...
$('#search_btn').click(function(){
    $('#mytab').bootstrapTable('refresh', {url: '/cooperationSubject/cooperationSubjectList'});
})
function refush(){
    $('#mytab').bootstrapTable('refresh', {url: '/cooperationSubject/cooperationSubjectList'});
}
$("#update").click(function(){
    $.post(
        "/cooperationSubject/cooperationSubjectUpdateSave",
...
$("#add").click(function(){
    $.post(
        "/cooperationSubject/cooperationSubjectAddSave",
        $("#formadd").serialize(),
        function(data){
...
function deleteMany(){
    ...
    layer.confirm('确认要执行批量删除网站信息数据吗？',function(index){
        $.post(
            "/cooperationSubject/deleteManyCooperationSubject",
            {
                "manyId":$("#deleteId").val()
            },
            function(data){
